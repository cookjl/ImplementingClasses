...
#   in file
#   m0_INSTRUCTIONS.txt, asking questions as needed.
#
...
class Point(object):
    ...
    def move_to(self,x,y):
        self.distance = self.distance + math.sqrt(((x - self.x) ** 2) + ((y -
                                                                  self.y) ** 2))
        self.x=x
        self.y=y
    ...
    def get_distance_from_start(self):
        return math.sqrt(((self.x-self.originalx)**2)+((
                self.y-self.originaly)**2))
    ...
    def halfway_to(self,p):
        ...
        hp=Point(self.x+(dx/2),self.y+(dy/2))
        return hp
